...
$(function () {
  ...
  chrome.storage.local.get(["options_host", "options_token"], function (data) {
    var options_host = data.options_host;
    var options_token = data.options_token;

    // 如果配置为空，提醒并跳转
    if (!options_host || !options_token) {
      alert("请初始化配置再上传!");
...
// uploader上传控制
function initUploader(options_host) {
  var uploader = new Dropzone(".dropzone", {
    ...
    parallelUploads: 20,
    dictDefaultMessage:
      '<button type="button" class="btn btn-primary">点击这里选择本机图片</button><br>或者<br>拖拽/黏贴您的文件到这里',
    dictFallbackMessage: "您的浏览器不支持拖拽......",
    dictFallbackText:
      "Please use the fallback form below to upload your files like in the olden days.",
    dictFileTooBig:
      "你传的玩意有 {{filesize}}MiB这么大.但是我就允许你传: {{maxFilesize}}MiB.",
...
console.log(
  "%c图床演示网站: https://png.cm \n请为本人博客 https://blog.png.cm/ 加上链接, 谢谢尊重!\n作为开发者你可以对相应的后台功能进行扩展(增删改相应代码), 但请保留代码中相关来源信息(例如: 本人博客, 邮箱等);\n本程序由 Icret 独自开发并完全开源, 碰到收费发布的请不要轻易付款; 本人仅为程序开源创作, 如非法网站使用与本人无关, 请勿用于非法用途.%c ",
  "color: #eaad1a; padding:5px 0; border:1px solid #448ef6; font-size:12px;",
  "",
);
